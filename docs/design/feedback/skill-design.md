...
```python
# src/openclaw_alpha/backend/feedback/submit_feedback.py
...
def submit_feedback(
    ...
    Args:
        ...
        source_user: 提交用户
        source_channel: 提交渠道
        source_session: 来源 session key（处理完成后用于发送结果消息）

    Returns:
        {"success": bool, "feedback_id": str, "file_path": str}
    """
    # 1. 生成反馈 ID
    feedback_id = hashlib.sha256(content.encode('utf-8')).hexdigest()

    # 2. 生成时间戳（Asia/Shanghai）
...
def main():
    ...
    parser.add_argument('--content', required=True, help='反馈原文内容')
...
**V1.1**：
- 支持反馈分类（bug / feature / improvement）
- 支持关联相关 skill（如 `related_skills: ["stock_analysis"]`）
- 支持优先级设置（由用户指定，默认 P2）

**V1.2**：
- 支持反馈附件（截图、日志文件）
- 支持反馈引用（回复之前的对话上下文）
- 支持匿名反馈（不记录 source_user）
