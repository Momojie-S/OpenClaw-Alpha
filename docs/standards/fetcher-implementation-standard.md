...
Fetcher 基类的 `fetch()` 方法支持自动降级：按优先级尝试每个可用的 Method，执行失败时降级到下一个。

**执行流程**：

1. 按优先级降序排序所有 Method
2. 遍历每个 Method：
   - 检查 `is_available()`（token、积分等前置条件）
   - 如果不可用，记录错误，继续下一个
   - 如果可用，执行 `method.fetch()`
   - 如果执行抛出**任何异常**，记录错误，继续下一个
   - 如果成功，返回结果
3. 全部失败时，抛出 `NoAvailableMethodError`，包含所有错误信息

**触发降级的场景**：

| 场景 | 示例 |
|------|------|
| 前置检查失败 | Token 未配置、积分不足 |
| 网络错误 | 超时、连接失败 |
| API 调用失败 | 服务端错误（5xx）、限流（429） |
| 数据源异常 | 返回数据格式错误、空数据 |
| 任何其他异常 | 代码 bug、未知错误 |

**注意**：**所有异常都会触发降级**，只有所有 Method 都失败时才抛出最终错误。
...
**⚠️ 必须检查所有引用该 Fetcher 的地方**

修改 Fetcher（包括接口签名、返回值结构、异常类型等）时，必须：

1. **查找所有引用**：
   ```bash
   # 搜索所有引用该 fetcher 的文件
   grep -r "from.*xxx_fetcher import" src/openclaw_alpha/skills/
   ```

2. **评估影响范围**：
   - 哪些 Processor 使用了这个 Fetcher
   - 是否有跨 skill 引用
   - 返回值结构变化是否影响下游处理逻辑
