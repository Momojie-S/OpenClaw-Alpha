...
async def fetch_stock_metrics(symbol: str, date: str) -> StockMetrics:
    ...
    Returns:
        StockMetrics
    """
    from skills.stock_analysis.scripts.stock_spot_fetcher.stock_spot_fetcher import (
        fetch_stock_spot,
    )

    # 获取行情数据
    spot = await fetch_stock_spot(symbol)
    ...
    return StockMetrics(
        ...
        amount=spot.get("amount", 0) / 1e8,  # 转为亿
        market_cap=spot.get("market_cap", 0) / 1e8,  # 转为亿
        pe=spot.get("pe"),
        pb=spot.get("pb"),
